...
class ZipCodeContract {
    ...
    public function getDataOfZipCodeAndDecode($zipCode){
        ...
        foreach($positions as $numberPointer){
            ...
            foreach($headers as $key => $header){
                $dataset[$header] = iconv("ISO-8859-1","ASCII//TRANSLIT",trim($values[$key]));
            }
            $zipCodes[] = $dataset;
        }
        return $zipCodes;
    ...
    public function getDataAndMapPrettyByZipCode($zipCode){
        ...
        $response = [
            "zip_code" => $zipCode,
            "locality" => strtoupper($datasets[0]["d_ciudad"]),
            "federal_entity" => [
                "key" => intval($datasets[0]["c_estado"]),
                "name" => strtoupper($datasets[0]["d_estado"]),
                "code" => null,
            ],
            "settlements" => [],
            "municipality" => [
                "key" => intval($datasets[0]["c_mnpio"]),
                "name" => strtoupper($datasets[0]["D_mnpio"])
            ],
        ];
        foreach($datasets as $dataset){
            $response["settlements"][] = [
                "key" => intval($dataset["id_asenta_cpcons"]),
                "name" => strtoupper($dataset["d_asenta"]),
                "zone_type" => strtoupper($dataset["d_zona"]),
                "settlement_type" => [
